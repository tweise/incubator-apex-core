...
public class StreamingContainerManagerTest
{
  ...
  public void testStaticPartitioning()
  {
    ...
    // unifier
    PTContainer containerWithUnifier;
    PTOperator unifier = null;

    for (PTContainer container : plan.getContainers()) {
      for (PTOperator operator : container.getOperators()) {
        if (operator.isUnifier()) {
          containerWithUnifier = container;
          unifier = operator;
          break;
        }
      }
    ...
    List<OperatorDeployInfo> cUnifier = getDeployInfo(dnm.getContainerAgent(unifier.getContainer().getExternalId()));
  ...
  public void testOperatorShutdown()
  {
    ...
    for (PTOperator ptOperator : physicalPlan.getAllOperators().values()) {
      if (ptOperator.isUnifier() && ptOperator.getName().startsWith(dag.getMeta(o2).getName())) {
        unifier = ptOperator;
        break;
      }
    }
